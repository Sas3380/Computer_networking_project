...
def reliable_UDP_receiver(window_size=5, enable_packet_loss=True):#by default 5 window size. 
    ...
    while True:
        ...
        if not verify_checksum(packet):
            ...
            # Discard corrupted packet by just ignoring it
            continue

        # extract sequence number (first 4 bytes) and message
        seq_num = struct.unpack('!I', packet[:4])[0]
        message_bytes = packet[4:-2]
        message = message_bytes.decode('utf-8')

        # if packet is inside the current window
